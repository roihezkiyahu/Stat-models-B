...
\\
OR = \frac{\frac{Pr(Y=1|X=x)}{1-Pr(Y=1|X=x)}}{\frac{Pr(Y=1|X=y)}{1-Pr(Y=1|X=y)}} = \frac{e^{\beta_{-k}^tx_{-k}}e^{\beta_kx_k}}{e^{\beta_{-k}^tx_{-k}}e^{\beta_kx_k}e^{\beta_k}} = e^{\beta_k}
$$
...
```{r}
model_ב <- glm(CVDeath_2012~Age,data = MI,family = "binomial")
```
